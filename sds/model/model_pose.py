...
def block_up(x: tf.Tensor, strides: IntOrTuple, ch_in: int, ch_out: int = 0, kernel_size: IntOrTuple = 4) -> Tuple[tf.Tensor, int]:
    strides = int_to_tuple(strides)
    if not ch_out:
        ch_out = ch_in // (strides[0] * strides[1])
...
def build_pose_layers_2d(input_size: int = 256, input_channels: int = 6, inp_pose_size: int = 6) -> Tuple[Tuple[tf.Tensor, tf.Tensor], Tuple[tf.Tensor, tf.Tensor]]:
    assert input_size == 256
    input_shape = input_size, input_size, input_channels
    inp = tf.keras.Input(input_shape)
    inp_pose = tf.keras.Input((inp_pose_size,), dtype=tf.float32)
    ...
    x, ch = block_down(x, 2, ch, ch * 2, 3) # 4, 4, 256
    x, ch = block_down(x, 2, ch, ch * 2, 3) # 2, 2, 512
    x, ch = layers.Conv2D(ch * 2, 2)(x), ch * 2 # 1, 1, 1024
    x_pos = tf.reshape(x, (-1, ch))

    x, ch = block_up(x, 2, ch, ch // 2, 2) # 2, 2, 512
    x, ch = block_up(x, 2, ch, ch // 2, 4) # 4, 4, 256
    x, ch = block_up(x, 2, ch, ch // 4, 4) # 8, 8, 128
    x, ch = block_up(x, 2, ch, ch // 4, 4) # 16, 16, 32
    x, ch = block_up(x, 2, ch, ch // 4, 4) # 32, 32, 8
    x, ch = block_up(x, 2, ch, ch // 4, 4) # 64, 64, 2
    x, ch = block_up(x, 2, ch, ch // 2, 4) # 128, 128, 1
